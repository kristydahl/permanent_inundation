...
def subtract_dems_from_wls(region):
    ...
    dems = arcpy.ListRasters('GA*final_DEM*') # change back to just *final_DEM for full analysis
    ...
    for dem in dems:
        ...
        outname_inundated_area_surface = 'inundated_area_surface_mhhw_012017' + '_' + dem[:-10] # remove date stamp for normal analysis
...
def combine_chunks (region):
    ...
    outname = 'merged_raw_raster_surface_mhhw_012017' #change back to not having date stamp for general analysis
...
def region_group(region, subregions):
    ...
    merged_raw_surface = arcpy.ListRasters('merged_raw_raster_surface_mhhw_012017')[0] # change back to not having date stamp for general analysis
...
def extract(region, subregions):
    ...
    if region == 'east_coast':
        ...
        for subregion in subregions:
            ...
            rg_surface = arcpy.ListRasters('rg_merged_raw_raster_surface_mhhw_012017_{0}' .format(subregion))[0] # change back to not having date stamp for general analysis
...
def raster_to_polygon(region, subregions):
    ...
    if region == 'east_coast':
        ...
        for subregion in subregions:
            ...
            to_convert = arcpy.ListRasters('extract_rg_merged*mhhw_012017_{0}' .format(subregion))[0] # change back to not having date stamp for general analysis
