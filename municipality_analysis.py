...
def clip_to_tracts_then_mhhw_polygon(region):
    ...
    #state_numbers = ['12','01','28','22'] #NEED TO DO 13 STILL
    ...
    for state_number in state_numbers:
        ...
        tracts = 'tl_2016_{0}_tract' .format(state_number)

        municipalities_clip_to_tracts = arcpy.Clip_analysis(municipalities, tracts, str(municipalities + '_clip'))
        #arcpy.Clip_analysis(municipalities_clip_to_tracts,'all_noaa_mhhw_mosaic_polygon',str(municipalities + '2_clip'))
        arcpy.Clip_analysis(municipalities_clip_to_tracts, 'all_noaa_mhhw_mosaic_polygon', str(municipalities + '3_clip'))
        print 'Clipped state municipalities to NOAA mhhw polygon for state number: ' + state_number

#Need to add Tot_area field to cousub2_clip for each state before running; NEED TO RUN 51 (VA)
def municipality_analysis_mhhw(region):
    ...
    #state_numbers = ['23','33','25','44','09','34','42','10','24','51','37','45','13','12','01','28','22','11']

    state_numbers = ['48']

    for state_number in state_numbers:

        #municipalities_clip_2 = 'tl_2016_{0}_cousub2_clip' .format(state_number)
        municipalities_clip_2 = 'tl_2016_{0}_cousub3_clip'.format(state_number)

        municipalities_layer = arcpy.MakeFeatureLayer_management(municipalities_clip_2, 'clipped_municipalities')

        #mhhw_inundation_surface = arcpy.ListFeatureClasses('final_polygon*mhhw_gulf_to_tx_clip_to_48')[0] # get into parameters

        state_mhhw_inundation_surface = arcpy.ListFeatureClasses('final_polygon*mhhw_gulf_to_tx_clip_to_48')[
            0]  # get into parameters
        #print mhhw_inundation_surface

        #outname_mhhw = str(mhhw_inundation_surface + '_clip_to_{0}' .format(state_number))
        #outname_mhhw = str(mhhw_inundation_surface + '_clip2_to_{0}'.format(state_number))
        #state_mhhw_inundation_surface = arcpy.Clip_analysis(mhhw_inundation_surface, 'clipped_municipalities', outname_mhhw)

        #state_mhhw_inundation_surface = arcpy.Clip_analysis(mhhw_inundation_surface, municipalities_clip_2,outname_mhhw)

        print 'Clipped inundation and mhhw surfaces to state'

        arcpy.AddField_management(municipalities_layer, "Area_MHHW", "FLOAT")
        #arcpy.AddField_management(municipalities_layer, "Pct_inun_MHHW","FLOAT")

        print 'Added Percent inundation and Area_MHHW fields'
        ...
        #arcpy.CalculateField_management(municipalities_layer, "Tot_area", exp)
        ...
        fields = ["SHAPE@", "ALAND", "STATEFP", "COUNTYFP", "NAME", "AWATER", "Tot_area", "Area_MHHW"]
        ...
        with arcpy.da.UpdateCursor('clipped_municipalities',fields) as cursor:
            ...
            for row in cursor:
                ...
                muni = row[0]
                muni_land_area = float(row[1])
                muni_state = row[2]
                muni_county = row[3]
                muni_name = row[4]
                muni_water_area = float(row[5])
...
def export_to_shapefile(years, projections, region, state_numbers):
    ...
    for state_number in state_numbers:
        ...
        to_export = arcpy.ListFeatureClasses('cohort_municipalities*{0}'.format(state_number))
...
#municipality_analysis_year(['2006'], ['NCAH'], 'east_coast', '26')

#cohort_id_shp(['2006','2030','2045','2060','2070','2080','2090','2100'],['NCAH'],'east_coast',['48'], 20)

export_to_shapefile([],[],'east_coast',['48'])
#cohort_id_shp(['2006','2030','2045','2060','2070','2080','2090','2100'],['NCAH'],'east_coast',['48'], 20)

#,'2045','2060','2070','2080','2090','2100'
